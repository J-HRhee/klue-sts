...
kluests_train = load_dataset("klue", "sts", split='train[:90%]')
kluests_valid = load_dataset("klue", "sts", split='train[-10%:]') # train의 10%를 validation set으로 사용
kluests_test = load_dataset("klue", "sts", split='validation')
...
korsts_train_examples = make_sts_input_example(korsts_train)
korsts_valid_examples = make_sts_input_example(korsts_valid)
korsts_test_examples = make_sts_input_example(korsts_test)
kluests_train_examples = make_sts_input_example(kluests_train)
kluests_valid_examples = make_sts_input_example(kluests_valid)
kluests_test_examples = make_sts_input_example(kluests_test)
# %%
# Train Dataloader
train_dataloader = DataLoader(
    korsts_train_examples+kluests_train_examples,
    shuffle=True,
...
dev_evaluator = EmbeddingSimilarityEvaluator.from_input_examples(
    korsts_valid_examples+kluests_valid_examples,
    name="kor_klue_sts-dev",
)

# Evaluator by sts-test
kor_test_evaluator = EmbeddingSimilarityEvaluator.from_input_examples(
    korsts_test_examples,
    name="kor_sts-test",
)

# Evaluator by korsts-test
klue_test_evaluator = EmbeddingSimilarityEvaluator.from_input_examples(
    kluests_test_examples,
    name="klue_sts-test",
)
# %%
...
if mode != 'train':
    ...
    # warmup steps
    warmup_steps = math.ceil(len(korsts_train_examples+kluests_train_examples) * sts_num_epochs / train_batch_size * 0.1) #10% of train data for warm-up
...
#%%
kor_test_evaluator(model, output_path=sts_model_save_path)
#%%
